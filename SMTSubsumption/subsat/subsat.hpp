...
public:
  explicit constexpr Var(uint32_t index) noexcept
      : m_index{index}
  {
      // assert(m_index <= Var::max_index());  // TODO: allow assert in constexpr (C++14 and later)
  }
...
private:
  ...
  explicit constexpr Lit(uint32_t index) noexcept
      : m_index{index}
  {
    assert(m_index <= Lit::max_index()); // TODO constexpr assert
  }
...
private:
  ...
  // (it does not allocate enough memory for the literals)
