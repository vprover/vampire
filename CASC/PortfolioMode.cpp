...
bool PortfolioMode::performStrategy(Shell::Property* property)
{
  ...
  Schedule schedule;
  // putting both quick and fallback into schedule
  getSchedules(*property,schedule,schedule);
  
  Schedule schedule_with_extras;
  getExtraSchedules(*property,schedule,schedule_with_extras,true /* add_extras */,1.0 /* multiply time*/);

  schedule.loadFromIterator(schedule_with_extras.iterFifo());
...
void PortfolioMode::runSlice(vstring sliceCode, int timeLimitInDeciseconds)
{
  ...
  try
  {
    ...
    // but we want each worker to have their own ...
    opt.setRandomSeed(getpid());
    // ... unless a strategy sets a seed explicitly, just here below
