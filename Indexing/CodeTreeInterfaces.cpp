...
template<bool higherOrder, class Data>
VirtualIterator<GenSubstitutionQR<Data>> CodeTreeTIS<higherOrder, Data>::getGeneralizations(TypedTermList t, bool retrieveSubstitutions) const
{
  if(_ct.isEmpty()) {
...
{
public:
  ResultIterator(const CodeTreeLIS& tree, Literal* lit, bool complementary, bool retrieveSubstitutions)
  : _retrieveSubstitutions(retrieveSubstitutions),
    _found(0), _finished(false), _tree(tree)
  {
...
private:
  ...
  const GenSubstitution<Data>* _subst;
  Recycled<Renaming> _resultNormalizer;
  bool _retrieveSubstitutions;
  Data* _found;
  bool _finished;
  const CodeTreeLIS& _tree;
  Recycled<typename LiteralCodeTree<Data>::LiteralMatcher> _matcher;
};

template<class Data>
VirtualIterator<GenSubstitutionQR<Data>> CodeTreeLIS<Data>::getGeneralizations(Literal* lit, bool complementary, bool retrieveSubstitutions) const
{
  if(_ct.isEmpty()) {
    return VirtualIterator<GenSubstitutionQR<Data>>::getEmpty();
  }
