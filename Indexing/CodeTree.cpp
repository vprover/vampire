...
bool CodeTree::Matcher<variant, removing>::execute()
{
  if(fresh) {
    fresh=false;
  }
...
template<bool variant, bool removing>
void CodeTree::Matcher<variant, removing>::init(CodeTree* tree_, CodeOp* entry_, LitInfo* linfos_, size_t linfoCnt_, Stack<CodeOp*>* firstsInBlocks_)
{
  tree=tree_;
  entry=entry_;
 ...
 */
template<bool variant, bool removing>
bool CodeTree::Matcher<variant, removing>::backtrack()
{
  if(btStack.isEmpty()) {
    curLInfo++;
...
template<bool variant, bool removing>
bool CodeTree::Matcher<variant, removing>::prepareLiteral()
{
  if constexpr (removing) {
...
template<bool variant, bool removing>
inline bool CodeTree::Matcher<variant, removing>::doAssignVar()
{
  ...
  if (fte->isVar()) {
    // TODO I think here we need to check the var range as well
    // in the variant==true case
    bindings[var]=TermList::var(fte->_number());
    tp++;
  }
  else {
    // in the variant==true case we are looking for variants
...
template<bool variant, bool removing>
inline bool CodeTree::Matcher<variant, removing>::doCheckVar()
{
...
template<bool variant, bool removing>
inline bool CodeTree::Matcher<variant, removing>::doCheckFun()
{
...
template<bool variant, bool removing>
inline bool CodeTree::Matcher<variant, removing>::doCheckGroundTerm()
{
...
template<bool variant, bool removing>
inline bool CodeTree::Matcher<variant, removing>::doSearchStruct()
{
