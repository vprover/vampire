...
public:
  ...
  UpwardChainingLHSIndex(TermIndexingStructure<TermLiteralClause>* is, const Ordering& ord, const Options& opt, bool left)
...
{
public:
  UpwardChainingSubtermIndex(TermIndexingStructure<TermLiteralClause>* is, const Ordering& ord, const Options& opt, bool left)
  : TermIndex(is), _ord(ord), _opt(opt), _left(left) {}
protected:
  void handleClause(Clause* c, bool adding) override;
  const Ordering& _ord;
