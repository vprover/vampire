...
void LoopAnalyzer::simpleSEIProblem()
{
  ...
  env.options->set("naming","32000");
  env.options->set("time_limit","10");
  //create the problem from the units generated by analyzing the loop
  Problem* prb= new Problem(_units);
  cout<<prb->getProperty()->toString()<<endl;
  Preprocess p(*env.options);
  p.preprocess(*prb);
  //just get the units after preprocess and print them - for testing purpose only
  List<Unit*>* u = prb->units();
  List<Unit*>::Iterator ite(u);
