...
public:
  ...
  virtual bool fulfilsAgeLimit(unsigned age, unsigned w, unsigned numeralWeight, unsigned length, bool derivedFromGoal, Inference* inference) const = 0;
  virtual bool fulfilsWeightLimit(Clause* cl) const = 0;
  // note: w here denotes the weight as returned by weight().
  // this method internally takes care of computing the corresponding weightForClauseSelection.
  virtual bool fulfilsWeightLimit(unsigned w, unsigned numeralWeight, unsigned length, bool derivedFromGoal, unsigned age, Inference* inference) const = 0;
