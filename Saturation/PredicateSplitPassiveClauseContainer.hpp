...
private:
  ...
  virtual float evaluateFeatureEstimate(unsigned age, unsigned w, unsigned numeralWeight, bool derivedFromGoal, unsigned numPositiveLiterals, Inference* inference) const = 0;
...
public:
  ...
  bool fulfilsAgeLimit(unsigned age, unsigned w, unsigned numeralWeight, bool derivedFromGoal, unsigned numPositiveLiterals, Inference* inference) const override;
  bool fulfilsWeightLimit(Clause* cl) const override;
  // note: w here denotes the weight as returned by weight().
  // this method internally takes care of computing the corresponding weightForClauseSelection.
  bool fulfilsWeightLimit(unsigned w, unsigned numeralWeight, bool derivedFromGoal, unsigned age, unsigned numPositiveLiterals, Inference* inference) const override;
...
private:
  ...
  float evaluateFeatureEstimate(unsigned age, unsigned w, unsigned numeralWeight, bool derivedFromGoal, unsigned numPositiveLiterals, Inference* inference) const override;
};
