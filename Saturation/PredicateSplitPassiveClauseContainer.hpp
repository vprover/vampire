...
public:
  ...
  bool fulfilsAgeLimit(unsigned age, unsigned w, unsigned numeralWeight, unsigned length, bool derivedFromGoal, Inference* inference) const override;
  bool fulfilsWeightLimit(Clause* cl) const override;
  // note: w here denotes the weight as returned by weight().
  // this method internally takes care of computing the corresponding weightForClauseSelection.
  bool fulfilsWeightLimit(unsigned w, unsigned numeralWeight, unsigned length, bool derivedFromGoal, unsigned age, Inference* inference) const override;
