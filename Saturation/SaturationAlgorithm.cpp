...
void SaturationAlgorithm::onNewClause(Clause* cl)
{
  ...
  if (cl->isPureTheoryDescendant()){
    ASS_REP(TheoryInstAndSimp::isTheoryLemma(cl) != TheoryInstAndSimp::TheoremStatus::CounterSat,cl->toString())
  }
#endif
...
SaturationAlgorithm *SaturationAlgorithm::createFromOptions(Problem& prb, const Options& opt, IndexManager *indexMgr)
{
  ...
  if (alascaTakesOver) {
    ...
    if (env.options->viras()) {
      if (env.options->alascaVariableEliminationGenerating())
        sgi->push(new ALASCA::VirasQuantifierEliminationSGI(shared, /* simplify */ false));
      else
        ise->addFrontMany(new ALASCA::VirasQuantifierEliminationISE(shared));
    } else {
      if (env.options->alascaVariableEliminationGenerating())
        sgi->push(new ALASCA::VariableEliminationSGI(shared, /* simplify */ false));
      else
