...
auto inequalityResolutionIdx(
  ...
  return Stack<std::function<Indexing::Index*()>>{
    [=]() { return new LascaIndex<InequalityResolution::Lhs>(uwa); },
    [=]() { return new LascaIndex<InequalityResolution::Rhs>(uwa); },
  }; 
}
...
{ return InequalityResolution(testLascaState(uwa)); }
...
REGISTER_GEN_TESTER(LascaGenerationTester<InequalityResolution>(testInequalityResolution()))
...
TEST_GENERATION(abstraction2,
    Generation::SymmetricTest()
      .rule(    new InequalityResolution(testInequalityResolution(Options::UnificationWithAbstraction::LASCA1))  )
...
TEST_GENERATION(abstraction1_irc2,
    Generation::SymmetricTest()
      .rule(    new InequalityResolution(testInequalityResolution(Options::UnificationWithAbstraction::LASCA2))  )
