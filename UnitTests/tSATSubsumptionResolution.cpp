...
TEST_FUN(SubsumptionResolutionWithAssumption)
{
  __ALLOW_UNUSED(SYNTAX_SUGAR_SUBSUMPTION_RESOLUTION);
  SATSubsumptionAndResolution subsumption;

  // Test 1
  Kernel::Clause* L1 = clause({ p(x1), p(x2) , ~q2(x1, x2) });
  Kernel::Clause* M1 = clause({ p(c), p(d), q2(c, d), q2(d,c)});
  ASS(!subsumption.checkSubsumptionResolutionWithLiteral(L1, M1, 0));
  ASS(!subsumption.checkSubsumptionResolutionWithLiteral(L1, M1, 1));
  ASS(subsumption.checkSubsumptionResolutionWithLiteral(L1, M1, 2));
  ASS(subsumption.checkSubsumptionResolutionWithLiteral(L1, M1, 3));

  // Test 2
  Kernel::Clause* L2 = clause({ p(x1), p(x2) , ~q2(x1, x2) });
  Kernel::Clause* M2 = clause({ p(c), p(d), q2(c, d)});
  ASS(!subsumption.checkSubsumptionResolutionWithLiteral(L2, M2, 0));
  ASS(!subsumption.checkSubsumptionResolutionWithLiteral(L2, M2, 1));
  ASS( subsumption.checkSubsumptionResolutionWithLiteral(L2, M2, 2));
}
