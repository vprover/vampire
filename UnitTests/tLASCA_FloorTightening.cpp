...
#include "Inferences/LASCA/FloorTightening.hpp"
...
FloorTightening testFloorTightening(
    Options::UnificationWithAbstraction uwa = Options::UnificationWithAbstraction::ALASCA1
    )
{ 
  return FloorTightening(testLascaState(uwa));
}

REGISTER_GEN_TESTER(Test::Generation::GenerationTester<FloorTightening>(testFloorTightening()))
...
TEST_GENERATION(basic10,
    Generation::SymmetricTest()
      .rule(new FloorTightening(testFloorTightening(Shell::Options::UnificationWithAbstraction::OFF)))
...
TEST_GENERATION(non_linear_tryout01,
    Generation::SymmetricTest()
      ...
      .rule(new FloorTightening(testFloorTightening(Shell::Options::UnificationWithAbstraction::LPAR_MAIN)))
      .expected(exactly(  
           clause({ 0 * (a * a) != 0 })
          )))
