...
HOL_TEST_FUN(eta_reduction_1) {
  ...
  term = toNameless(lam({x0, x1}, app({D.f2, x0, x1})));
  ASS_EQ(termListToString(term, Options::HPrinting::TPTP),
  ...
  term = toNameless(lam({x0, x1, x2}, app({D.f3, x0, x1, x2})));
  ASS_EQ(termListToString(term, Options::HPrinting::TPTP),
...
HOL_TEST_FUN(eta_reduction_2) {
  auto term = toNameless(lam({x0, x1, x2}, app({D.f3, x0, x2, x1})));
...
HOL_TEST_FUN(eta_reduction_3) {
  auto v0 = Defs::x(0, AtomicSort::arrowSort(D.srt, D.srt, D.srt));
  auto term = toNameless(lam({v0, x1, x2}, app({v0, x1, x2})));
...
HOL_TEST_FUN(eta_reduction_4) {
  auto term = toNameless(lam(x0, app({D.f2, x0, x0})));

  ASS_EQ(etaNF(term), term)
}

HOL_TEST_FUN(eta_reduction_5) {
  auto v0 = Defs::x(0, D.fSrt);
  auto term = toNameless(lam(v0, app(D.g, lam(x1, app(v0, x1)))));
