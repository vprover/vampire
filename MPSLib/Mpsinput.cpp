   ...
   while( readLine() )
   {
      ...
      if( strcmp(colname, f1) )
      {
         ...
         if( isInteger )
         {
            ...
            model->pushVar(new Variable(colname, Variable::BINARY, 0.0, 1.0, 0.0));
         }
         else
         {
            /* for continuous variables, default bounds are 0 <= x, and default cost is 0 */
            model->pushVar(new Variable(colname, Variable::CONTINUOUS, 0.0, INFBOUND, 0.0));
         }
      }
