...
{
  return _po.set(x, y, PoComp::GREATER);
}
...
string VarOrder::to_string() const
{
...
Stack<const VarOrder*> VarOrder::order_diff(const VarOrder* vo, const VarOrder* other)
{
...
Stack<const VarOrder*> VarOrder::order_diff_nonrecursive(const VarOrder* vo, const VarOrder* other)
{
  auto tr = other->transitive_reduction();

  Stack<const VarOrder*> res;
  // VarOrder temp = vo;
  ...
  while (List<Edge>::isNonEmpty(tr)) {
    ...
    switch (e.c) {
      case PoComp::GREATER:
        if (vo->query(e.x,e.y) != PoComp::GREATER) {
          auto eq = VarOrder::add_eq(vo,e.x,e.y);
          auto lt = VarOrder::add_gt(vo,e.y,e.x);
          ASS(eq);
          ASS(lt);
          res.push(eq);
          res.push(lt);
          ALWAYS(vo = VarOrder::add_gt(vo,e.x,e.y));
        }
        break;
      case PoComp::EQUAL:
        if (vo->query(e.x,e.y) != PoComp::EQUAL) {
          auto gt = VarOrder::add_gt(vo,e.x,e.y);
          auto lt = VarOrder::add_gt(vo,e.y,e.x);
          ASS(gt);
          ASS(lt);
          res.push(gt);
          res.push(lt);
          ALWAYS(vo = VarOrder::add_eq(vo,e.x,e.y));
        }
        break;
      default:
        ASSERTION_VIOLATION;
...
{
  static VarOrder empty;
  return &empty;
}

const VarOrder* VarOrder::add_gt(const VarOrder* vo, unsigned x, unsigned y)
{
...
const VarOrder* VarOrder::add_eq(const VarOrder* vo, unsigned x, unsigned y)
{
  // cout << "querying " << vo->to_string() << " " << x << " = " << y << endl;
  const VarOrder** ptr;
...
const VarOrder* VarOrder::tryExtendWith(const VarOrder* vo, const VarOrder* other)
{
  static DHMap<pair<const VarOrder*,const VarOrder*>,const VarOrder*> cache;
  const VarOrder** ptr;
  if (!cache.getValuePtr(make_pair(vo,other),ptr,nullptr)) {
    return *ptr;
  }
  ...
  while (List<Edge>::isNonEmpty(tr)) {
    auto e = tr->head();
    if (e.c == PoComp::EQUAL) {
      vo = VarOrder::add_eq(vo,e.x,e.y);
      if (!vo) {
        break;
      }
    } else if (e.c == PoComp::GREATER) {
      vo = VarOrder::add_gt(vo,e.x,e.y);
      if (!vo) {
        break;
      }
    } else if (e.c == PoComp::LESS) {
      vo = VarOrder::add_gt(vo,e.y,e.x);
      if (!vo) {
        break;
      }
    }
