...
// unidirectional comparison function (returns correct result if tt1 > tt2 or tt1 = tt2)
Ordering::Result LPO::lpoIterative(AppliedTerm a1, AppliedTerm a2) const
{
  struct Call {
    Result type;
    AppliedTerm tt1;
    AppliedTerm tt2;
    const TermList* args1;
    const TermList* args2;
  };
  Recycled<Stack<Call>> calls;
  calls->push({ LESS, a1, a2, nullptr, nullptr });
  Result comp;

  auto pushNext = [&](Result r) {
    calls->pop();
    while (calls->isNonEmpty()) {
      auto& [type, tt1, tt2, args1, args2] = calls->top();
      ASS_NEQ(type, LESS);
      args1--;
      args2--;
      switch (type) {
  ...
  while (calls->isNonEmpty()) {
    ...
    switch (type) {
      case LESS: {
        ...
        if (tt1.term.isVar()) {
          pushNext((tt1.term==tt2.term) ? EQUAL : INCOMPARABLE);
          continue;
        }

        if (tt2.term.isVar()) {
          pushNext(tt1.containsVar(tt2.term) ? GREATER : INCOMPARABLE);
          continue;
        }
      ...
      case EQUAL: {
        if (args1->isNonEmpty()) {
          calls->push({ LESS, AppliedTerm(*args1,tt1), AppliedTerm(*args2,tt2), nullptr, nullptr });
        } else {
          pushNext(EQUAL);
        }
        break;
      ...
      case GREATER: {
        if (args2->isNonEmpty()) {
          calls->push({ LESS, tt1, AppliedTerm(*args2, tt2), nullptr, nullptr });
        } else {
          pushNext(GREATER);
        }
        break;
      ...
      case INCOMPARABLE: {
        if (args1->isNonEmpty()) {
          calls->push({ LESS, AppliedTerm(*args1,tt1), tt2, nullptr, nullptr });
        } else {
          pushNext(INCOMPARABLE);
        }
        break;
      }
    }
