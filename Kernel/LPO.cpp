...
bool LPO::isGreater(TermList tl1, TermList tl2, void* tl1State, VarOrderBV* constraints, const Indexing::TermQueryResult* qr) const
{
  ...
  ASS(tl1.isTerm());
  static const VarOrder vo;
  return clpo_gt(tl1.term(), tl2, vo);
}

bool LPO::isGreater(TermList tl1, TermList tl2, const VarOrder& vo) const
{
  TIME_TRACE("LPO::isGreater2");
  if (tl1==tl2) {
    return false;
  }
  if (tl1.isVar()) {
    return tl2.isVar() && vo.query(tl1.var(),tl2.var())==PoComp::GT;
  }
  ASS(tl1.isTerm());
  return clpo_gt(tl1.term(), tl2, vo);
}

bool LPO::clpo_gt(Term* t1, TermList tl2, const VarOrder& vo) const
{
  ASS(t1->shared());

  if(tl2.isVar()) {
    VariableIterator vit(t1);
    while (vit.hasNext()) {
      auto v = vit.next().var();
  ...
  switch (comparePrecedences(t1, t2)) {
  case EQUAL:
    return lexMAE_gt(t1, t2, t1->args(), t2->args(), t1->arity(), vo);
  case GREATER:
    return majo_gt(t1, t2->args(), t2->arity(), vo);
  case LESS:
    return alpha_gt(t1->args(), t1->arity(), t2, vo);
  default:
    ASSERTION_VIOLATION;
  }
}
...
// greater iff some exists s_i in sl such that s_i >= t 
bool LPO::alpha_gt(TermList* sl, unsigned arity, Term *t, const VarOrder& vo) const
{
  ASS(t->shared());

  for (unsigned i = 0; i < arity; i++) {
    if (*(sl - i) == TermList(t) || lpo_gt(*(sl - i), TermList(t), vo)) {
      return true;
    }
  }
  return false;
...
bool LPO::lpo_gt(TermList tl1, TermList tl2, const VarOrder& vo) const
{
  ASS_NEQ(tl1,tl2); // we want to check this outside
...
bool LPO::lexMAE_gt(Term* s, Term* t, TermList* sl, TermList* tl, unsigned arity, const VarOrder& vo) const
{
  ASS(s->shared());
  ASS(t->shared());
  ...
  for (unsigned i = 0; i < arity; i++) {
    ...
    if (lpo_gt(*(sl - i), *(tl - i), vo)) {
      return majo_gt(s, tl - i - 1, arity - i - 1, vo);
    } else {
      return alpha_gt(sl - i - 1, arity - i - 1, t, vo);
    }
  }
  // reached only when the terms are equal but this is checked already
  // at the start of LPO::lpo, which is the only caller of this function
...
// greater if s is greater than every term in tl
bool LPO::majo_gt(Term* s, TermList* tl, unsigned arity, const VarOrder& vo) const
{
  ASS(s->shared());

  for (unsigned i = 0; i < arity; i++) {
    if (TermList(s) == *(tl - i) || !lpo_gt(TermList(s), *(tl - i), vo)) {
      return false;
    }
  }
  return true;
...
bool LPO::makeGreater(TermList tl1, TermList tl2, VarOrder& vo) const
{
  TIME_TRACE("LPO::makeGreater");
  if (tl1==tl2) {
    return false;
  }
  if (tl1.isVar()) {
  ...
  ASS(tl1.isTerm());
  return clpo_mgt(tl1.term(), tl2, vo);
}

bool LPO::clpo_mgt(Term* t1, TermList tl2, VarOrder& vo) const
{
  ASS(t1->shared());
  ...
  switch (comparePrecedences(t1, t2)) {
  case EQUAL:
    return lexMAE_mgt(t1, t2, t1->args(), t2->args(), t1->arity(), vo);
  case GREATER:
    return majo_mgt(t1, t2->args(), t2->arity(), vo);
  case LESS:
    return alpha_mgt(t1->args(), t1->arity(), t2, vo);
  default:
    ASSERTION_VIOLATION;
  }
}
...
// greater iff some exists s_i in sl such that s_i >= t 
bool LPO::alpha_mgt(TermList* sl, unsigned arity, Term *t, VarOrder& vo) const
{
  ASS(t->shared());

  for (unsigned i = 0; i < arity; i++) {
    if (*(sl - i) == TermList(t) || lpo_mgt(*(sl - i), TermList(t), vo)) {
      return true;
    }
  }
  return false;
...
bool LPO::lpo_mgt(TermList tl1, TermList tl2, VarOrder& vo) const
{
  ASS_NEQ(tl1,tl2); // we want to check this outside
...
bool LPO::lexMAE_mgt(Term* s, Term* t, TermList* sl, TermList* tl, unsigned arity, VarOrder& vo) const
{
  ASS(s->shared());
  ASS(t->shared());
  ...
  for (unsigned i = 0; i < arity; i++) {
    ...
    if (lpo_mgt(*(sl - i), *(tl - i), vo)) {
      return majo_mgt(s, tl - i - 1, arity - i - 1, vo);
    } else {
      return alpha_mgt(sl - i - 1, arity - i - 1, t, vo);
    }
  }
  // reached only when the terms are equal but this is checked already
  // at the start of LPO::lpo, which is the only caller of this function
...
// greater if s is greater than every term in tl
bool LPO::majo_mgt(Term* s, TermList* tl, unsigned arity, VarOrder& vo) const
{
  ASS(s->shared());

  for (unsigned i = 0; i < arity; i++) {
    if (TermList(s) == *(tl - i) || !lpo_mgt(TermList(s), *(tl - i), vo)) {
      return false;
    }
  }
  return true;
