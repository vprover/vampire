...
namespace Kernel {
  ...
  private:
    ...
    static Ordering::Result _mulExt(MultiSet<A> const& ls, MultiSet<A> const& rs, Cmp cmp_, Option<MulExtMemo&> memo)
    {
      CALL("mulExt")
  ...
  public:
    ...
    template<class A, class Cmp> 
    static Ordering::Result mulExt(MultiSet<A> const& l, MultiSet<A> const& r, Cmp cmp)
    { MulExtMemo memo; return mulExt(l, r, cmp, memo); }


    template<class A, class Cmp> 
    static Ordering::Result mulExtWithoutMemo(MultiSet<A> const& l, MultiSet<A> const& r, Cmp cmp)
    { return _mulExt(l, r, cmp, Option<MulExtMemo&>()); }

    template<class A, class Cmp> 
    static Ordering::Result mulExt(MultiSet<A> const& l, MultiSet<A> const& r, Cmp cmp, MulExtMemo& memo)
