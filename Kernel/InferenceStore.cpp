...
protected:
  ...
  void printStep(Unit* us)
  {
    ...
    if(us->isClause() && static_cast<Clause*>(us)->inputType() < Unit::CONJECTURE){

      unsigned depth = us->inference()->maxDepth();
      if(depth > max_theory_clause_depth){
        max_theory_clause_depth=depth;
      }
    }
  }
