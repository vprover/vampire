...
/**
 * Get the parents of unit represented by us and fill in the inference used to generate this unit
 *
 ...
 */
UnitIterator InferenceStore::getParentsAndInference(Unit* us, Inference*& inf)
{
  CALL("InferenceStore::getParentsAndInference");
  ASS_NEQ(us,0);
...
/**
 * Get parents and the generating rule
 */
UnitIterator InferenceStore::getParents(Unit* us, InferenceRule& rule)
{
  ...
  Inference* inf;
  UnitIterator res = getParentsAndInference(us, inf);
  rule = inf->rule();
  return res;
}

/**
 * Get parents where we do not care about the generating rule nor the inference
 */
UnitIterator InferenceStore::getParents(Unit* us)
{
  ...
  Inference* aux;
  return getParentsAndInference(us, aux);
}
...
struct InferenceStore::ProgramPrinter
{
  CLASS_NAME(InferenceStore::ProgramPrinter);
  USE_ALLOCATOR(InferenceStore::ProgramPrinter);
  
  ProgramPrinter(ostream& out, InferenceStore* is)
  : _is(is), _out(out), _subs(List<pair<RobSubstitution*, int>>::empty()), _indent(0), _inf(nullptr) {}

  ~ProgramPrinter() {}

  void printIndent() {
    for (unsigned i = 0; i < _indent; ++i) _out << "  ";
  }

  void applySubs(TermList& t) {
    auto subit = List<pair<RobSubstitution*, int>>::Iterator(_subs);
    while (subit.hasNext()) {
      const auto subAndIdx = subit.next();
      //if (!t.isVar() || !subAndIdx.first->isUnbound(t.var(), subAndIdx.second)) {
      //cout << "applying " << subAndIdx.first->toString();
      //cout << "  from " << t << " ";
      t = subAndIdx.first->apply(t, subAndIdx.second);
      //cout << "to " << t << endl;
      //}
    }
    if (t.isTerm() && _inf && _inf->hasProgramIVR()) {
      t = _inf->applyProgramIVR(t);
    }
  }

  void applySubs(Literal*& l) {
    auto subit = List<pair<RobSubstitution*, int>>::Iterator(_subs);
    while (subit.hasNext()) {
      const auto subAndIdx = subit.next();
      l = subAndIdx.first->apply(l, subAndIdx.second);
    }
  }

  void print(Unit* us) {
    CALL("InferenceStore::ProgramPrinter::print");
    //cout << "printing " << us->toString() << endl;
    Inference* inf;
    UnitIterator parents = _is->getParentsAndInference(us, inf);
    vvector<Unit*> p;
    while (parents.hasNext()) p.push_back(parents.next());
    // TODO: make it work for induction
    if (p.size() == 0) {
      if (inf->programHasVariable()) {
        printIndent();
        TermList var = inf->getProgramResult();
        TermList result = var;
        // TODO: here we don't pass the right inf. Also even if we did maybe it wouldn't work since it cannot transform into vars???
        applySubs(result);
        printIndent();
        _out << var << ": " << result << endl;
      } else {
        printIndent();
        _out << "ERR / Program should be only variable, but it is TL/var: " << inf->getProgramResult() << "/" << inf->getProgramTargetVar() << " for unit: " << us->toString() << endl;
      }
      // TODO: check if the program in 'inf' is a statement containing only the variable
      // if not, maybe just print "???" and what's wrong
      // else ASS and then apply all _subs in the order and print _indent*"  " + "\var = \result"
    } else if ((p.size() == 1) || (inf->rule() == InferenceRule::SKOLEMIZE)) {
      // TODO maybe this can also be a loop?
      // TODO: check if it's a "statement" containing a substitution
      // if not, print what's wrong
      // else add the substitution to _subs, call print(p[0]), and remove the subst from _subs
      bool hasSub = inf->programHasSubstitution();
      if (hasSub) {
        const RobSubstitutionSP& sub = inf->getProgramSubstitution();
        _subs = List<pair<RobSubstitution*, int>>::cons(make_pair(sub.ptr(), 0), _subs);
        //cout << "pushing sub " << sub->toString();
      }
      ...
      if (hasSub) { List<pair<RobSubstitution*, int>>::pop(_subs);}
      //} else {
      //  printIndent();
      //  _out << "ERR? / No substitution for unit: " << us->toString() << endl;
      //}
    } else if (p.size() == 2) {
      // TODO: check if it's an ITE containing a subst
      // if not, print what's wrong
      // else add the subst to _subs, print _indent*"  "+"if \cond then\n", _indent++, call print(p[0]), print (_indent-1)*"  "+"else", call print(p[1]), _indent--, remove the subst from _subs
      if (inf->programHasCondition()) {
        if (inf->programHasSubstitution()) {
          const RobSubstitutionSP& sub = inf->getProgramSubstitution();
          //cout << "pushing sub " << sub->toString();
          _subs = List<pair<RobSubstitution*, int>>::cons(make_pair(sub.ptr(), 0), _subs);
          printIndent();
          Literal* cond = inf->getProgramCondition();
          applySubs(cond);
          _out << "IF " << cond->toString() << " THEN" << endl;
          _indent++;
          print(p[0]);
          cout << "pop"<< endl; List<pair<RobSubstitution*, int>>::pop(_subs);
          _indent--;
          printIndent();
          _out << "ELSE" << endl;
          _indent++;
          //cout << "pushing sub " << sub->toString();
          _subs = List<pair<RobSubstitution*, int>>::cons(make_pair(sub.ptr(), 1), _subs);
          print(p[1]);
          cout << "pop"<< endl; List<pair<RobSubstitution*, int>>::pop(_subs);
          _indent--;
        } else {
          _out << "ERR / No substitution for ITE: " << us->toString() << endl;
        }
      } else {
        _out << "ERR / No condition for unit: " << us->toString() << endl;
      }
      // TODO: check the order of p0, p1 in printing
    } else {
      _out << "ERR / cannot handle unit with " << p.size() << " premises: " << us->toString() << endl;
      // TODO: this we cannot handle yet, print info
    }
     
  }

  List<pair<RobSubstitution*, int>>* _subs;
  unsigned _indent;
  InferenceStore* _is;
  ostream& _out;
  Inference* _inf;  // not owned, always check if not nullptr
};
...
void InferenceStore::outputProof(ostream& out, Unit* refutation)
{
  ...
  ScopedPtr<ProgramPrinter> prog(new ProgramPrinter(out, this));
  // TODO: print this out in ProgramPrinter
  out << "% Program:" << endl;
  prog->print(refutation);
}
