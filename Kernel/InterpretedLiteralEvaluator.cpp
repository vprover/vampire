...
struct Polynom {
  struct Coeff {
    union Content {
      ...
      Content() {}
      Content(IntegerConstantType i) : _int(i){}
      Content(RationalConstantType r) : _rat(r) {}
      Content(RealConstantType r) : _real(r) {}
      operator IntegerConstantType(){ return _int; }
      operator RationalConstantType(){ return _rat; }
      operator RealConstantType(){ return _real; }
    } _cont;
    enum Tag {
      Int,
      Rat,
    ...
    Coeff(IntegerConstantType i) : _cont(i), _tag(Int){  }
    Coeff(RationalConstantType r) : _cont(r), _tag(Rat) {}
    Coeff(RealConstantType r) : _cont(r), _tag(Real) {}
    Coeff(){}
    ~Coeff() {
      switch (_tag){
        case Int: 
    ...
    Coeff(Coeff&& other) {
      ...
      switch (_tag) {
        case Int: 
          _cont._int = std::move(other._cont._int);
          break;
        case Rat: 
          _cont._rat = std::move(other._cont._rat);
          break;
        case Real: 
          _cont._real = std::move(other._cont._real);
          break;
      }
    }
    ...
    Coeff& operator=(Coeff&& other) {
      CALL("Coeff::operator=(Coeff&&)")
      DBG("moving coeff: ", other)
      _tag = other._tag;
      switch (_tag) {
        case Int: 
          _cont._int = std::move(other._cont._int);
    ...
    friend std::ostream& operator<<(std::ostream& out, const Coeff& self) {
      switch (self._tag) {
        case Int: return out << self._cont._int;
        case Rat: return out << self._cont._rat;
        case Real: return out << self._cont._real;
      }
    }
  };
  ...
  template<class ConstantType>
  void multiply(ConstantType c) {
    CALL("Polynom::multiply")
    _const = c * _const._cont;
    for (auto& pair : _coeffs) {
      pair.second = c * pair.second._cont;
    }
  }
  ...
  void set(TermList t, Const c) {
    ...
    if (iter == _coeffs.end()) {
      // _coeffs.insert({t, c});
      _coeffs.insert(decltype(_coeffs)::value_type( t, c ));
    } else {
      _coeffs[t] = c;
    }
  }
  ...
  template<class ConstantType>
  Polynom(ConstantType c) : _const(c), _coeffs(decltype(_coeffs)())  { 
    DBG("Polynom(ConstantType) -> ", *this)
  }
  ...
  Polynom& operator=(Polynom&& other) {
    ...
    DBG("mvAsgn");
    DBG("moving: ", *this , " <- " , other);
    _const = std::move(other._const);
    DBG("moving coeffs...");
    // for (auto& c : other._coeffs)
    _coeffs = std::move(other._coeffs);
    DBG("finished moving coeffs");
    return *this;
  }
  // Polynom& operator=(Polynom&& other)  {
  //   _const = std::move(other._const);
...
//     // *p = std::move(Kernel::TermEvalResult::left(Kernel::TermList()));
//     ASSERTION_VIOLATION
//     // *p = std::move(Kernel::TermEvalResult::left(Kernel::TermList()));
//   }
//   template<class T>
//   // using rebind_alloc = alloc<T>;
//   using rebind_alloc = vector<Kernel::TermEvalResult>::allocator_type;
...
// template<>
// Kernel::TermEvalResult::Either() { ASSERTION_VIOLATION }

namespace Kernel {


  // class TermEvalResult : public Either<Kernel::TermList, Kernel::Polynom> {
  //   TermEvalResult() : Either() {ASSERTION_VIOLATION}
  // };
      ...
      if (theory->isInterpretedFunction(&inner)) {
        switch (theory->interpretFunction(inner.functor())) {
          case number::addI:
            ...
            // TODO de-recursify
            return number::add(
                _evaluateUnaryMinus<number>(inner[0]),
                _evaluateUnaryMinus<number>(inner[1])
                );
          case number::mulI:
            // NORMALIZATION
            return number::mul(
...
#define IMPL_UNARY_MINUS(Const) \
  template<> TermEvalResult NewEvaluator::evaluateFun<num_traits<Const>::minusI>(Term* orig, TermEvalResult* evaluatedArgs) const  \
  { \
...
#define IMPL_EVAL_UNARY(Const, INTER, expr) \
  template<> TermEvalResult NewEvaluator::evaluateFun<Interpretation::INTER>(Term* orig, TermEvalResult* evaluatedArgs) const   \
...
template<class ConstantType, class EvalGround>
TermEvalResult NewEvaluator::tryEvalConstant1(Term* orig, TermEvalResult* evaluatedArgs, EvalGround fun) const {
...
template<class ConstantType, class EvalGround>
TermEvalResult NewEvaluator::tryEvalConstant2(Term* orig, TermEvalResult* evaluatedArgs, EvalGround fun) const {
...
// TermEvalResult NewEvaluator::evaluateStep(TermList term) const {
//   if (term.isTerm()) {
...
TermList NewEvaluator::evaluate(Term* term) const {
  ...
  ASS(args.size() == 1);
  TermEvalResult out = TermEvalResult::rightMv( Polynom() );
...
inline TermList createTerm(unsigned fun, const Signature::Symbol& sym, TermEvalResult* evaluatedArgs) {
  ...
  for (int i = 0; i < arity; i++) {
    args.push(evaluatedArgs[0].toLeft(
      ...
      // [&](const Polynom& p) { 
      //   switch (op.arg(i)) {
      //     case Sorts::SRT_INTEGER: return p.toTerm<num_traits<IntegerConstantType>>();
      //     case Sorts::SRT_RATIONAL: return p.toTerm<num_traits<RationalConstantType>>();
      //     case Sorts::SRT_REAL: return p.toTerm<num_traits<RealConstantType>>();
...
#define HANDLE_NUM_CASES(NUM) \
  ...
  // const unsigned sort = sym->fnType()->result();
