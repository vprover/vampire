...
bool ColorHelper::hasOnlyColoredConstants(Clause* c)
{
  CALL("ColorHelper::hasOnlyColoredConstants");
...
void ColorHelper::collectColoredConstants(Clause* c, Stack<Term*>& acc)
{
  ...
  for(unsigned i=0; i<clen; i++) {
    ...
    while(tit.hasNext()) {
      Term* t = tit.next().term();
      if(t->arity()==0 && !isTransparent(false, t->functor())) {
	acc.push(t);
      }
    }
  }
...
Clause* ColorHelper::skolemizeColoredConstants(Clause* c)
{
  ...
  static Stack<Term*> coloredConstants;
  coloredConstants.reset();
