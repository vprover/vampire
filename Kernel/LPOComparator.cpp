...
LPOComparator2::LPOComparator2(TermList lhs, TermList rhs, const LPO& lpo)
...
ostream& operator<<(ostream& out, const LPOComparator2::BranchTag& t)
{
  switch (t) {
    ...
    case LPOComparator2::BranchTag::T_COMPARISON:
      out << "$";
      break;
    case LPOComparator2::BranchTag::T_UNKNOWN:
      out << "?";
      break;
  }
  return out;
...
std::ostream& operator<<(std::ostream& str, const LPOComparator2::Branch& branch)
{
  str << branch.tag << " ";
  if (branch.n) {
...
std::string LPOComparator2::toString() const
{
  std::stringstream str;
  str << *this << endl;
  return str.str();
...
void LPOComparator2::expand(Branch& branch, const LPO& lpo)
{
...
bool LPOComparator2::check(const SubstApplicator* applicator)
{
  const auto& lpo = static_cast<const LPO&>(_ord);
  auto curr = &_root;
