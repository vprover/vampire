...
#endif
  /** returns and id unique to this sort. These ids are independent of term.getId(). These ids are a bijective mappings between `TermList`s that represent sorts and integers in [0,count()[ */
  unsigned getSortNum(TermList sort);
  /** returns the sort represented b the given id. The id must be within [0,count()[. */
