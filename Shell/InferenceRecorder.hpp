...
class InferenceRecorder{
    ...
    void backwardDemodulation(unsigned int id, Kernel::Clause* conclusion, const std::vector<Kernel::Clause*>& premises, const Indexing::ResultSubstitutionSP& recordedSubst);

	void setCurrentGoal(Kernel::Clause* goal) { _currentGoal = goal; _lastInferenceId = 0;}

	const InferenceInformation* getLastRecordedInferenceInformation() const {
		auto it = _inferences.find(_lastInferenceId);
		if(it != _inferences.end()) return it->second.get();
		return nullptr;
	}
