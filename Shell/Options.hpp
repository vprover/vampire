...
#define NUMBER_OPTION_PARSER(num, parseFunction)                                          \
  ...
  struct OptionParser<num> {                                                              \
                                                                                          \
    static Option<num> parse(std::string const& value) {                                      \
      num out;                                                                            \
      return someIf(parseFunction(value.c_str(),out), [&]() { return out; });             \
    }                                                                                     \
                                                                                          \
    static std::string display(num value)                                                     \
...
template<>
struct OptionParser<std::string> {
  static Option<std::string> parse(std::string const& value) 
  { return some(value == "<empty>" ? "" : std::string(value)); }

  static std::string display(std::string const& value) 
  { return value.empty() ? "<empty>" : value; }
};


template<>
struct OptionParser<Ratio> {
  static Option<Ratio> parse(std::string const& value) 
  { 
  ...
  static std::string display(Ratio const& value) 
  { 
...
{
  static Option<bool> parse(std::string const& value) {
...
{
  static Option<Option<T>> parse(std::string const& value) {
    if (value == "<nothing>") {
  ...
  static std::string display(Option<T> value) 
...
private:
    ...
    struct TypedOptionValue : public AbstractOptionValue {
        ...
        virtual std::string getStringOfValue(T value) const = 0;
    ...
    struct OptionValue : public TypedOptionValue<T> {
      ...
      virtual std::string getStringOfValue(T value) const override
...
public:
  ...
  Ratio ageWeightRatio() const { return _ageWeightRatio.actualValue; }
