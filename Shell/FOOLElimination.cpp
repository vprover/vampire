      ...
      case SpecialFunctor::LET: {
        ...
        if (Theory::isTupleConstructor(bindingLhs)) {
          ...
          if (bindingRhs.isTerm() && Theory::isTupleConstructor(bindingRhs.term())) {
            ...
            unsigned tupleSymbol = env.signature->addFreshFunction(OperatorType::getConstantsType(tupleResultSort, tupleTypeArgs.size()), LET_PREFIX)->number();
...
unsigned FOOLElimination::introduceFreshSymbol(Context context, const char* prefix,
  ...
  unsigned symbol;
  if (context == FORMULA_CONTEXT) {
    symbol = env.signature->addFreshPredicate(type, prefix)->number();
  } else {
    symbol = env.signature->addFreshFunction(type, prefix)->number();
  }
