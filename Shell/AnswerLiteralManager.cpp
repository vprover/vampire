...
Unit* AnswerLiteralManager::tryAddingAnswerLiteral(Unit* unit)
{
  ...
  if (skolemise) {
    ...
    while (fvit.hasNext()) {
      ...
      unsigned skFun = env.signature->freshFunction(OperatorType::getConstantsType(sort), "sK", /*suffix=*/"in").skolem().number();
...
TermList AnswerLiteralManager::possiblyEvaluateAnswerTerm(TermList aT)
{
  if(aT.isTerm() && !aT.term()->isSpecial()){
    ...
    unsigned p = env.signature->freshPredicate(OperatorType::getPredicateType({sort}),"p").number();
...
Term* SynthesisALManager::translateToSynthesisConditionTerm(Literal* l)
{
  ...
  bool added = false;
  unsigned fn = env.signature->function(fnName, OperatorType::getFunctionType(argSorts, AtomicSort::defaultSort()), added).number();
...
TermList getConstantForVariable(TermList sort) {
  ...
  if (sort == AtomicSort::intSort()) {
    ...
    if (!env.signature->tryGetFunctionNumber(name, 0, czfn)) {
      czfn = env.signature->freshFunction(OperatorType::getConstantsType(sort), name.c_str()).number();
    } 
...
SynthesisALManager::ConjectureSkolemReplacement::Function::Function(unsigned recFunctor, ConjectureSkolemReplacement* replacement) {
  ...
  _functor = env.signature->freshFunction(OperatorType::getFunctionType({in}, out), "rf").number();
