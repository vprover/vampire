...
struct DoSubstitution : public Transformation<DoSubstitution> {
  ...
  DoSubstitution(Substitution &substitution) : substitution(substitution) {}
...
static std::ostream &operator<<(std::ostream &out, DkSplit dk) {
  CloseParens cp(out);
  return out << cp.open_if(!dk.split.positive(), "(not ") << "sp" << dk.split.var();
}
...
void outputPrelude(std::ostream &out) {
  ...
  for(unsigned i = Signature::FIRST_USER_CON; i < sig.typeCons(); i++) {
    auto *sym = sig.getTypeCon(i);
    outputTypeDecl(out, sym->name(), sym->typeConType());
  }

  for(unsigned i = 0; i < sig.functions(); i++) {
    auto *sym = sig.getFunction(i);
    ASS(!sym->interpreted())
    outputTypeDecl(out, sym->name(), sym->typeConType());
  }

  for(unsigned i = 1; i < sig.predicates(); i++) {
    auto *sym = sig.getPredicate(i);
    ASS(!sym->interpreted())
    outputTypeDecl(out, sym->name(), sym->typeConType());
  }
}
