...
//*********************** Saturation  ***********************
    ...
    _numClauseFeatures.onlyUsefulWith(_neuralClauseEvaluationModel.is(notEqual(vstring(""))));
    ...
    _neuralClauseEvaluationModelGSD.onlyUsefulWith(_neuralClauseEvaluationModel.is(notEqual(vstring(""))));
    ...
    _neuralPassiveClauseContainer.reliesOn(Or(_neuralActivityRecordingModel.is(notEqual(vstring(""))), _neuralClauseEvaluationModel.is(notEqual(vstring("")))));
    ...
    _neuralActivityRecordingModel.reliesOn(_neuralActivityRecording.is(notEqual(vstring(""))));
    ...
    _neuralPassiveClauseContainer.reliesOn(_neuralClauseEvaluationModel.is(notEqual(vstring(""))));
    ...
    _ageWeightRatioShapeFrequency.addHardConstraint(greaterThan(0u)); // we divide by this value in AWPPassiveClauseContainer::popSelected
    ...
    _sineToAgeGeneralityThreshold.onlyUsefulWith(Or(
      _neuralClauseEvaluationModel.is(notEqual(vstring(""))),
  ...
  //*********************** Inferences  ***********************
    ...
    _unificationWithAbstraction = ChoiceOptionValue<UnificationWithAbstraction>("unification_with_abstraction","uwa",
                                     ...
                                     {"off","interpreted_only","one_side_interpreted","constant","all","ground","ac1","ac2"});
...
//*********************** SAT solver (used in various places)  ***********************
    ...
    "Select the SAT solver to be used throughout the solver. This will be used in AVATAR (for splitting) when the saturation algorithm is discount,lrs or otter."
     " (For instance generation for selection and global subsumption, we currently only use minisat.)";
    ...
    _nonGoalWeightCoefficient = NonGoalWeightOptionValue("nongoal_weight_coefficient","nwc",10.0);
