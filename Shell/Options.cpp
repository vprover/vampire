...
//*********************** Saturation  ***********************
  ...
  //*********************** Inferences  ***********************
    ...
    _eqToIneq.tag(OptionTag::INFERENCES);
    _lookup.insert(&_eqToIneq);

    _inequalityChaining = BoolOptionValue("unit_ineq_chaining","uic",true);
    _inequalityChaining.description = "chains unit inequalities";
    ...
    _chainUnrolling = BoolOptionValue("chain_unrolling","cu",false);
    _chainUnrolling.description = "Unrolls a chain with a ground term as length parameter one step at a time"
                                  "                   C[chain(loc,tp,len)]            "
                                  "             --------------------------------      "
    ...
    _extensionalityResolution.description=
      ...
      "If malloc the only extensionality clauses that we recognise are those of the form malloc(l#(X)) != malloc(l#(Y)) \\/ X = Y";      
...
//*********************** SAT solver (used in various places)  ***********************
    ...
    _symbolPrecedence = ChoiceOptionValue<SymbolPrecedence>("symbol_precedence","sp",SymbolPrecedence::ARITY,
                                                            {"arity","occurrence","rev_occ","reverse_arity","unary_first",
                                                            "const_max", "const_min",
    ...
    _symbolPrecedence.description="Vampire uses term orderings which require a precedence relation between symbols.\n"
                                  ...
                                  "reverse occurence orders symbols in the reverse of the order they occur in"
