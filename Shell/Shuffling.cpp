...
void Shuffling::polarityFlip(Problem& prb)
{
  ...
  while (us.hasNext()) {
    ...
    // std::cout << "Before: " << cl->toString() << std::endl;
    newLits.reset();
    bool modified = false;
    for (unsigned i = 0; i < cl->length(); i++) {
      Literal* l = (*cl)[i];
      // std::cout << "  bef: " << l->toString() << std::endl;
      if (flippage[l->functor()]) {
        l = Literal::complementaryLiteral(l);
        modified = true;
      }
      newLits.push(l);
      // std::cout << "  aft: " << l->toString() << std::endl;
    }
    // std::cout << "After: " << cl->toString() << std::endl;
    if (modified) {
...
{
  // std::cout << "Bef: " << unit->toString() << std::endl;
  ...
  // std::cout << "Aft: " << unit->toString() << std::endl;
}
    ...
    if (sh.is<Formula*>()) {
      ...
      switch (fla->connective() ) {
        ...
        case EXISTS:
        {
          ...
          //std::cout << "Shuffling FORALL/EXISTS: " << fla->toString() << std::endl;
          ...
          //std::cout << "getting: " << fla->toString() << std::endl;
