...
{
  auto sigma = [&](auto t) { return sigma_.applyTo(t, /* var bank */ 0); };
  auto j = j_s1.numeral;
  auto k = k_s2.numeral;

  Stack<Literal*> conclusion(hyp1->size() + hyp2->size() + cnst.size());

  auto C1 = iterTraits(hyp1->getLiteralIterator()).filter([&](auto lit) { return lit != lit1; });
  auto C2 = iterTraits(hyp2->getLiteralIterator()).filter([&](auto lit) { return lit != lit2; });
  ...
  auto resolvent2 = NumTraits::eq(true, sigma(l1.term().denormalize()), NumTraits::zero());
  //   ^^^^^^^^^^----> (js1 + t1 ≈ 0)σ
