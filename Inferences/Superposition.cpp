...
template<bool higherOrder>
ClauseIterator Superposition<higherOrder>::generateClausesWithNonGoalSuperposableTerms(Clause* premise, TypedTermList t)
{
  ASS(t.isTerm());
  ...
  return pvi(iterTraits(NonVariableNonTypeIterator(t.term(), /*includeSelf=*/true))
    ...
    .flatMap(
      ...
      { return pushPairIntoRightIterator(arg, _lhsIndex->getUwa<higherOrder>(arg.first, _salg.getOptions())); })
...
template<bool higherOrder>
ClauseIterator Superposition<higherOrder>::performForwardSuperpositions(Clause* premise)
{
...
template<bool higherOrder>
ClauseIterator Superposition<higherOrder>::performBackwardSuperpositions(Clause* premise)
{
  return pvi(premise->getSelectedLiteralIterator()
