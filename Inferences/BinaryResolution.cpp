...
Clause* BinaryResolution::generateClause(Clause* queryCl, Literal* queryLit, SLQueryResult qr, const Options& opts, PassiveClauseContainer* passiveClauseContainer, Ordering* ord, LiteralSelector* ls)
{
  ...
  bool needsToFulfilWeightLimit = passiveClauseContainer && !passiveClauseContainer->fulfilsAgeLimit(newAge, wlb, numPositiveLiteralsLowerBound, infSp.ptr()) && passiveClauseContainer->weightLimited();
  if(needsToFulfilWeightLimit) {
    ...
    if(!passiveClauseContainer->fulfilsWeightLimit(wlb, newAge, numPositiveLiteralsLowerBound, infSp.ptr())) {
  ...
  for(unsigned i=0;i<clength;i++) {
    ...
    if(curr!=queryLit) {
      ...
      if(needsToFulfilWeightLimit) {
        ...
        if(!passiveClauseContainer->fulfilsWeightLimit(wlb, newAge, numPositiveLiteralsLowerBound, res->inference())) {
