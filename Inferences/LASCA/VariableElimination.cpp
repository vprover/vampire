      ...
      //                            ^^^-->  t1 + ... + tn <> 0 
        ...
        for (auto s : lit.term().iterSummands()) {
          ...
          if (var.isSome()) {
            ...
            // lit = t1 + ... + k * f(...) + ... + tn <> 0  
            VariableIterator vit(s.factors.denormalize());
            while (vit.hasNext()) {
              shielded.insert(vit.next().var());
            }
          }
        }
      });
...
ClauseIterator VariableElimination::applyRule(Clause* premise, FoundVariable<NumTraits> found) const
{
  ...
  auto b = [x](auto& i) {
    return iterTraits(i.literal.term().iterSummands())
      ...
      .map([&](auto t) { return (i.literal.symbol() == LascaPredicate::EQ || i.literal.symbol() == LascaPredicate::NEQ) && i.numeral.isNegative()
                                ? -t : t; });
  };
  ...
  { 
    ...
    for (auto& i : I) {
      for (auto& j : J) {
        auto gr = i.literal.symbol() == LascaPredicate::GREATER && j.literal.symbol() == LascaPredicate::GREATER 
          ?  NumTraits::greater
          : NumTraits::geq;
...

  {
    ...
    for (auto& i : I) {
      for (auto& l : L) {
        auto gr = i.literal.symbol() == LascaPredicate::GREATER ? NumTraits::greater
                                                              : NumTraits::geq;
...

  {
    ...
    for (auto& j : J) {
      for (auto& l : L) {
        auto gr = j.literal.symbol() == LascaPredicate::GREATER ? NumTraits::greater
                                                              : NumTraits::geq;
  ...
  return pvi(iterTraits(partitionIter(std::move(K)))
      .map([common = std::move(common), 
        ...
        Inference inf(GeneratingInference1(Kernel::InferenceRule::LASCA_VARIABLE_ELIMINATION, premise));
