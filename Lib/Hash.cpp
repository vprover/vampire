 ...
 * @since 31/03/2006
 *
 * Now the FNV hash is being used for null-terminated strings
 * regardless the USE_LOOKUP3_HASH macro, as the lookup3 algorithm
 * doesn't provide a function for hashing null-terminated strings.
 */
...
unsigned Hash::hash(const unsigned char* data,size_t length)
{
#if HASH_LITTLE_ENDIAN
  return hashlittle(data,length,0);
#else
  return hashbig(data,length,0);
#endif
}
unsigned Hash::hash(const unsigned char* data,size_t length,unsigned begin)
{
#if HASH_LITTLE_ENDIAN
  return hashlittle(data,length,begin);
#else
  return hashbig(data,length,begin);
#endif
}
unsigned Hash::hashWords(const unsigned* data, size_t length)
{
  return hashword(data,length,0);
}
