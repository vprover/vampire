...
bool collectionOfObjects::insertVariableExpression(std::string key, Program::VariableExpression* obj){
   CALL("collectionOfObjects::insertVariableExpression");
  _mapOfVariableExpr.insert(key, obj);
  return true;
}

Program::VariableExpression* collectionOfObjects::getVarExpression(std::string key){
CALL("getVariableExpression");
  Program::VariableExpression* varE = NULL;
  _mapOfVariableExpr.find(key, varE);
  if(varE==NULL)
  {
    Program::Variable* vv=  (new Program::Variable(key, Program::Type::integerType()));
...
void collectionOfObjects::insertConstantIntegerExpr(int val, Program::ConstantIntegerExpression* obj){
CALL("insertConstrantIntegerExpression");
  _mapOfIntegers.insert(val, obj);
}


void collectionOfObjects::insertAssignment(std::string key, Program::Assignment* obj){
...
Program::ConstantIntegerExpression* collectionOfObjects::getConstantIntegerExpr(int key){
...
void collectionOfObjects::insertFunctionApplication(std::string key, Program::FunctionApplicationExpression* obj){
...
Program::FunctionApplicationExpression* collectionOfObjects::getFunctionApplicationExpression(std::string key){
...
void collectionOfObjects::insertIfThenElse(std::string key, Program::IfThenElse* obj){
...
void collectionOfObjects::insertArrayApplication(std::string key, Program::ArrayApplicationExpression* obj){
 CALL("insertArrayApplication");
  _mapOfArrayApplic.insert(key, obj);
}

Program::ArrayApplicationExpression* collectionOfObjects::getArrayApplicationExpression(std::string key){
...
if(_mapOfAssignments.find(key)) return 1;
else
if(_mapOfWhile.find(key)) return 2;
else 
  if(_mapOfThenElse.find(key)) return 3;
...
void collectionOfObjects::runAnalysis(int whileNumber){
	CALL("collectionOfObjects::runAnalysis");
	  ...
	  if(_mapOfWhile.numberOfElements() < whileNumber || whileNumber <= 0)
		  ...
		  int whileNo=1;
			  Program::Statement* s;
	  Lib::Map<int, Program::Statement*>::Iterator mpi(_mapOfMainProgramStatements);
...
void collectionOfObjects::insertMainProgramStatement(int number, Program::Statement* stmt){
...
void collectionOfObjects::insertMainProgramStatement(Program::Statement* stmt){
	CALL("collectionOfObjects::insertMainProgramStatement");
	_mapOfMainProgramStatements.insert(_mapOfMainProgramStatements.numberOfElements()+1, stmt);
}

void collectionOfObjects::insertVariable(std::string key, Program::Variable* obj){
