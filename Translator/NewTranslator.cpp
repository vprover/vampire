...
void newTranslator::VisitDeclStmt(const DeclStmt* stmt) {
  ...
  CALL("newTranslator::VisitDeclStmt");
    ...
    if (stmt->isSingleDecl()) {
         ...
         for(::clang::DeclGroupRef::const_iterator ci= stmt->decl_begin();
                ci != stmt->decl_end(); ci++)
         {
          decl = (*ci);
          treatSingleDeclaration(decl);
...
void newTranslator::VisitStmt(const Stmt* stmt)
{
  CALL("newTranslator::visitStmt");
    for (::clang::Stmt::const_child_iterator it = stmt->child_begin();
            it != stmt->child_end(); ++it)
    {
            ...
            if (::clang::IfStmt::classof(*it))
        {
            ...
            Visit(ifStmt->getCond());
                ...
                if(flag==false)
                Body.insert(Body.end(), bac);
                else colect->insertMainProgramStatement(colect->getIfThenElse(bac));

       } else
        {
            //all the other cases (assignments, initialization expressions)
            Visit(*it);
            if (!::clang::DeclStmt::classof(*it) && flag == true && ! clang::ReturnStmt::classof(*it))
            {
  ...
  CALL("newTranslator::VisitBinAssign");
    ...
    else
    {
        ...
        if(isVariable(bop->getLHS()))
        {
        	...
        	Program::FunctionApplicationExpression* fc= colect->getFunctionApplicationExpression(numeUitat);
          ass = new Program::Assignment(v, fc);
        }
          else{
          ae=colect->getArrayApplicationExpression(nn);
          ass = new Program::Assignment(ae, colect->getFunctionApplicationExpression(numeUitat));
        }
...
CALL("newTranslator::VisitUnaryMinus");
    ...
    Program::FunctionApplicationExpression* tempFC = new Program::FunctionApplicationExpression(Program::ConstantFunctionExpression::integerNegation());
  ...
  CALL("newTranslator::isNumber");
    ...
    if (::clang::UnaryOperator::classof(exp))
    {
        ...
        if (uop->isArithmeticOp())
        {
            ...
            if (::clang::ImplicitCastExpr::classof(uop->getSubExpr())) return false;
...
void newTranslator::AddArrayName(::std::string name){
  ...
  if(!colect->findVarExpression(name))
  {
    Program::ArrayType* arr= new Program::ArrayType(Program::Type::integerType());
    Program::Variable* tempVar = new Program::Variable(name, arr);
    Program::VariableExpression* tempVarE = new Program::VariableExpression(tempVar);
    colect->insertVariableExpression(name, tempVarE);
  }
...
void newTranslator::AddValues(int val){
  ...
  if(!colect->findConstantInteger(val)){
    Program::ConstantIntegerExpression* con= new Program::ConstantIntegerExpression(val);
...
void newTranslator::RunRewriting()
{
  CALL("newTranslator::RunRewriting");
    ...
    for (uint i = 0; i < _mainProgram.size();i++)
    {
      ...
      {
      case 1:  mainBlock->setStatement(i, colect->getAssignment(_mainProgram[i]));
...
CALL("newTranslator::writeWhileStatements");
    ...
    { int j;
        ...
        if (b_while != -1 && e_while != -1)
        {
            ...
            //condition, body
            Program::FunctionApplicationExpression* condit=colect->getFunctionApplicationExpression(Body[b_while + 1]);
    ...
    if (flag == true) {
		...
		colect->insertBlock("loop" + _mainProgram[_mainProgram.size() - 1],
				wBlock);
		while (!Body.empty())
			Body.pop_back();
...
void newTranslator::writeIfStatments(string att)
{
  CALL("newTranslator::writeIfStatements");
    ...
    //go to first then - outer if
        ...
        ite= new Program::IfThenElse(colect->getFunctionApplicationExpression(Body[b_inIf+1]), _thenP, _elseP);
...
::std::string newTranslator::getSignedValue(const ::clang::Expr *exp)
{
  CALL("newTranslator::getSignedValue");
    ...
    if (::clang::UnaryOperator::classof(exp))
    {
        ...
        {
             ::clang::IntegerLiteral *l = (::clang::IntegerLiteral *)uop->getSubExpr();
             APSInt val;
    ...
    if (::clang::ImplicitCastExpr::classof(exp))
    {
        ...
        const ::clang::ImplicitCastExpr *impc = (const ::clang::ImplicitCastExpr *)exp;
...
void newTranslator::treatSpecialCase(const ::clang::Expr *lhs, ::std::string operation)
{
  CALL("newTranslator::treatSpecialCase");
    ...
    if(operation== "PlusPlus"){
      newop =  "integerPlus";
      fapp= new Program::FunctionApplicationExpression(Program::ConstantFunctionExpression::integerPlus());
    }
      else{
      fapp= new Program::FunctionApplicationExpression(Program::ConstantFunctionExpression::integerMinus());
      newop= "integerMinus";
        }
    ...
    if(variable == NULL)
    {
      Program::Variable* vv = new Program::Variable(c0, Program::Type::integerType());
    ...
    else numeUitat = c0 + "_ASS_" + construction;

}



void newTranslator::treatSpecial(const ::clang::Expr *lhs, const ::clang::Expr *rhs, ::std::string operation)
{
CALL("newTranslator::treatSpecial");
    ...
    if (operation == "PlusEq" || operation == "MinusEq" || operation == "MultEq")
    {
        ...
        Program::FunctionApplicationExpression* fapp = new Program::FunctionApplicationExpression(getConstFunction(newop));
        ...
        if (isSimpleExpression(rhs))
        {
          ...
          if(isNumber(rhs)){
            ...
            Program::ConstantIntegerExpression* cie =colect->getConstantIntegerExpr(s);
            fapp->setArgument(1, cie);
          }
          else if(isVariable(rhs)){
        	  Program::VariableExpression* varE= colect->getVarExpression(construction);
        	  fapp->setArgument(1, varE);
          }
          else{
        	  Program::ArrayApplicationExpression* arr= colect->getArrayApplicationExpression(construction);
        ...
        else //si mai trebuie adaugate expresiile in map.
        {
            ...
            Program::FunctionApplicationExpression* f = colect->getFunctionApplicationExpression(numeUitat);
            fapp->setArgument(1, f);

        }

        if(!colect->findFunctionApplication(c0 + "_" + newop + "_" + construction))
        	colect->insertFunctionApplication(c0 + "_" + newop + "_" + construction, fapp);

        Program::Assignment* ass = new Program::Assignment(colect->getVarExpression(c0), fapp);
        if(!colect->findAssignemt(c0+"_ASS_"+c0 + "_" + newop + "_" + construction))
        	colect->insertAssignment(c0+"_ASS_"+c0 + "_" + newop + "_" + construction, ass);

        	if (flag == false)
            Body.insert(Body.end(),c0+"_ASS_"+c0 + "_" + newop + "_" + construction);
        else numeUitat = c0+"_ASS_"+c0 + "_" + newop + "_" + construction;
...
std::string newTranslator::WriteSimpleAssignment(const ::clang::Expr* lhs, const Expr* rhs, string operation){
	...
	bool flg=true;
			...
			{
				Program::Variable* v = new Program::Variable(numeUitat, Program::Type::integerType());
			...
			if(isNumber(rhs))
			{
				...
				{
					Program::Variable* va= new Program::Variable(name, Program::Type::integerType());
				...
				else ass= new Program::Assignment(arra,rightE);
			}else{
...
std::string newTranslator::simpleExpression(const clang::Expr* exp)
{
	...
	if(isNumber(exp))
	{
       ...
       if(!colect->findConstantInteger(val))
		{
    	Program::ConstantIntegerExpression* value= new Program::ConstantIntegerExpression(val);
	...
	if(isArray(exp))
	{
        if (::clang::ArraySubscriptExpr::classof(exp))
        {
            const ::clang::ArraySubscriptExpr *s = dyn_cast< ::clang::ArraySubscriptExpr > (exp);
            ...
            {
                const ::clang::ImplicitCastExpr *ice = dyn_cast< ::clang::ImplicitCastExpr > (base);
            ...
            if (isSimpleExpression(index))
            {
                ...
                if(isNumber(index))
                {
                  ...
                  stringstream ss(idx);
                  ss>>val;
                  Program::ConstantIntegerExpression* ct=colect->getConstantIntegerExpr(val);
                  arrApp= new Program::ArrayApplicationExpression(tmp, ct);

                }
                else if(isVariable(index)){
                  arrApp= new Program::ArrayApplicationExpression(colect->getVarExpression(nme),
            ...
            else
            {
              //constraint: for the moment we can handle only FunctionApplicationExpressions
                ...
                arrApp = new Program::ArrayApplicationExpression(colect->getVarExpression(nme), colect->getArrayApplicationExpression(numeUitat));
                if(!colect->findArrayApplication(name))
                	colect->insertArrayApplication(name, arrApp);
        ...
        if (::clang::ImplicitCastExpr::classof(exp)) {
			const ::clang::ImplicitCastExpr *e = dyn_cast<
					::clang::ImplicitCastExpr> (exp);
			::std::string composition = "", idx = "", nme = "";
			Program::ArrayApplicationExpression* arrApp;
...
::std::string newTranslator::WriteSimpleStatement(const ::clang::Expr *lhs, const ::clang::Expr *rhs, ::std::string operation)
{
	...
	else
	{
		...
		if (isNumber(lhs)) {
			...
			if (colect->findConstantInteger(stringToInt(name)))
				...
				numb = new Program::ConstantIntegerExpression(stringToInt(
								name));
				colect->insertConstantIntegerExpr(stringToInt(name), numb);
			}
		...
		Program::VariableExpression* rightE;
			if (colect->findVarExpression(name))
				...
				cout<<colect->getVarExpression("a")->toString(1)<<colect->getVarExpression(name)->toString(2)<<endl;
		...
		{
			Program::ArrayApplicationExpression* arra = colect->getArrayApplicationExpression(name);
...
Program::ConstantFunctionExpression* newTranslator::getConstFunction(std::string op){
  
 CALL("newTranslator::getCosntFunction");
  if( op=="integerEq") return Program::ConstantFunctionExpression::integerEq();
  else
  if(op=="integerGreater") return Program::ConstantFunctionExpression::integerGreater();
  else
    if(op=="integerGreaterEq") return Program::ConstantFunctionExpression::integerGreaterEq();
    else
      if(op=="integerLess") return Program::ConstantFunctionExpression::integerLess();
      else
        if(op=="integerLessEq") return Program::ConstantFunctionExpression::integerLessEq();
        else
          if(op=="integerMinus")return Program::ConstantFunctionExpression::integerMinus();
          else
            if(op=="integerMult")return Program::ConstantFunctionExpression::integerMult();
            else
              if(op=="integerPlus") return Program::ConstantFunctionExpression::integerPlus();
            return NULL;
}



std::string newTranslator::treatSimpleBinaryOperation(const clang::Expr* lhs, const clang::Expr* rhs, std::string op){

	CALL("newTranslator::treatSimpleBinaryOperation");
	Program::FunctionApplicationExpression* fcapp = new Program::FunctionApplicationExpression(
			getConstFunction(op));
	string name="", namer="";
	bool flg=false;
	...
	if(flg)
		{
			...
			else fcapp->setArgument(0, colect->getArrayApplicationExpression(name));
	...
	if(flg)
		{
			...
			else fcapp->setArgument(1, colect->getArrayApplicationExpression(namer));
