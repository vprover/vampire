...
protected:
   ...
   string WriteSimpleAssignment(const ::clang::Expr* lhs, const Expr* rhs, string operation);
   string WriteSimpleStatement(const ::clang::Expr *lhs, const ::clang::Expr *rhs, ::std::string operation);
	...
	void treatSpecial(const Expr *lhs, const Expr *rhs,
			string op);
	std::string treatSimpleBinaryOperation(const clang::Expr* lhs, const clang::Expr*, std::string op);
